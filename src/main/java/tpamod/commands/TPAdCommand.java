...
public class TPAdCommand extends ModularChatCommand {
    ...
    public void runModular(Client client, Server server, ServerClient serverClient, Object[] args, String[] errors, CommandLog logs) {
        ...
        if (target != null) {
            // 点对点拒绝：指定特定玩家的请求
            GameEvents.triggerEvent(new TPAResponseEvent(serverClient, target, false), (TPAResponseEvent e) -> {
                accepted.set(true);
            });
        } else {
            // 无参数：拒绝所有待处理请求（保持向后兼容）
            GameEvents.triggerEvent(new TPAResponseEvent(serverClient, null, false), (TPAResponseEvent e) -> {
                accepted.set(true);
            });
        }
